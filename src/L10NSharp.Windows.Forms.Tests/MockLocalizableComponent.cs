...
namespace L10NSharp.Windows.Forms.Tests
{
	class MockLocalizableComponent: ILocalizableComponent, IComponent
	{
		...
		public IEnumerable<LocalizingInfoWinforms> GetAllLocalizingInfoObjects(L10NSharpExtender extender)
		{
			var result = new List<LocalizingInfoWinforms>();
			foreach (var kvp in StringContainer)
			{
				var control = kvp.Key.Item1;
				var id = extender.GetLocalizingId(control) + kvp.Key.Item2;
				result.Add(new LocalizingInfoWinforms(control, id) { Text = kvp.Value, Category = LocalizationCategory.LocalizableComponent});
			}
			return result;
		}
...
#region unused IComponent support
		...
		// needed to implement the IComponent interface
