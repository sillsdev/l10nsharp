	...
	internal class LocalizationManagerInternalWinforms<T> : LocalizationManagerInternal<T>
	{
			...
			params string[] namespaceBeginnings)
		{
			...
			return Create(desiredUiLangId, appId, appName,
				...
				directoryOfWritableXliffFiles =>
					(ILocalizationManagerInternalWinforms<T>)new XliffLocalizationManagerWinforms(
						appId, origExeExtension, appName,
		...
		public static void ReapplyLocalizationsToAllObjectsInAllManagers()
		{
			...
			foreach (var lm in LoadedManagers.Values)
				...
				if (lm is ILocalizationManagerInternalWinforms<T> winformsLm)
					winformsLm.ReapplyLocalizationsToAllComponents();
		}
		...
		{
			var lm = GetLocalizationManagerForComponentWinforms(ctrl);
