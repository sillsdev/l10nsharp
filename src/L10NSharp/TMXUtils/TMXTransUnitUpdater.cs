...
{
	internal class TMXTransUnitUpdater
	{
		...
		/// ------------------------------------------------------------------------------------
		internal TMXTransUnitUpdater(TMXDocument tmxDoc)
		{
			_tmxDoc = tmxDoc;

			var replacement = _tmxDoc.Header.GetPropValue(TMXLocalizedStringCache.kHardLineBreakReplacementProperty);
			if (replacement != null)
