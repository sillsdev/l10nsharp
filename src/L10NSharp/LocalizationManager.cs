...
namespace L10NSharp
{
	...
	public class LocalizationManager : IDisposable
	{
		...
		private void CreateOrUpdateDefaultXliffFileIfNecessary(params string[] namespaceBeginnings)
		{
			...
			if (UILanguageId == "en" && IgnoreExistingEnglishXliffFiles && DefaultStringFileExistsAndHasContents())
				File.Delete(DefaultStringFilePath);

			if (!DefaultStringFileExistsAndHasContents() && File.Exists(defaultStringFileInstalledPath))
			{
				if (UILanguageId != "en" || !IgnoreExistingEnglishXliffFiles)
				...
				if (xliffOld != null)
				{
					...
					else
					{
						...
						if (tuOld.Dynamic && !tu.Dynamic)
						{
							...
							tu.AddNote("en", "Found in current static scan of compiled code");
						}
					}
				}
			}
					...
					if (tuNew == null)
					{
						...
						if (tu.Dynamic)
						{
							...
							if (newDynamicCount > 0)	// note only if attempt made to collect dynamic strings
								tu.AddNote("en", "Not found when running compiled program");
						}
						else
						{
							++missingStringCount;
							missingStringIds.Add(tu.Id);
							tu.AddNote("en", "Not found in current scan of compiled code");
						}
					}
				}
			}
