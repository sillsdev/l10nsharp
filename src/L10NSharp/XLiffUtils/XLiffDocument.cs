...
namespace L10NSharp.XLiffUtils
{
	...
	public class XLiffDocument
	{
		...
		{
			if(isTargetNeeded)
				return File.Body.TransUnits.Select(tu => tu.Sources).Select(v => v.Lang).Union(File.Body.TransUnits.Select(tu => tu.Targets).Where(x => x.Lang != null).Select(v => v.Lang)).Distinct();
			else
				return File.Body.TransUnits.Select(tu => tu.Sources).Select(v => v.Lang);
		}
