	...
	internal class LocalizedStringCache
	{
		...
		private void MergeXliffFilesIntoCache(IEnumerable<string> xliffFiles)
		{
			...
			foreach (var tu in DefaultXliffDocument.File.Body.TransUnits)
				DefaultXliffDocument.File.Body.TranslationsById.Add(tu.Id, tu.GetVariantForLang(LocalizationManager.kDefaultLang).Value);
			...
			foreach (var file in xliffFiles)
			{
				try
				{
					...
					var lookupTarget = xliffDoc.File.Body.TranslationsById;
					foreach (var tu in xliffDoc.File.Body.TransUnits)
					{
						...
						else
						{
							...
							if (target != null && !String.IsNullOrEmpty(target.Value))
								lookupTarget.Add(tu.Id, target.Value);
						}
//#if DEBUG
