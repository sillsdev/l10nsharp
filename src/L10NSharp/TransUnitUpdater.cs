...
namespace L10NSharp
{
	internal class TransUnitUpdater
	{
		...
		internal bool Update(LocalizingInfo locInfo)
		{
			...
			Dictionary<string, TransUnit> lookupSource;
			if (!_stringCache.LookupDictsForTransUnits.TryGetValue(_defaultLang, out lookupSource))
			{
				lookupSource = new Dictionary<string, TransUnit>();
				_updated = true;
				_stringCache.LookupDictsForTransUnits.Add(locInfo.LangId, lookupSource);
			}
			Dictionary<string, TransUnit> lookupTarget;
			if (!_stringCache.LookupDictsForTransUnits.TryGetValue(locInfo.LangId, out lookupTarget))
			{
				lookupTarget = new Dictionary<string, TransUnit>();
				_updated = true;
				_stringCache.LookupDictsForTransUnits.Add(locInfo.LangId, lookupTarget);
			}
			...
			if (locInfo.Priority == LocalizationPriority.NotLocalizable)
			{
				...
				RemoveFromLookupCache(lookupSource, tuSourceText);
				RemoveFromLookupCache(lookupTarget, tuSourceText);
				RemoveFromLookupCache(lookupSource, tuSourceToolTip);
				RemoveFromLookupCache(lookupTarget, tuSourceToolTip);
				RemoveFromLookupCache(lookupSource, tuSourceShortcutKeys);
				RemoveFromLookupCache(lookupTarget, tuSourceShortcutKeys);
				return _updated;
			}

			// Save the shortcut keys
			TransUnit tuTargetShortcutKeys = null;
			var shortcutId = locInfo.Id + kShortcutSuffix;
			if ((locInfo.UpdateFields & UpdateFields.ShortcutKeys) == UpdateFields.ShortcutKeys)
			{
				tuTargetShortcutKeys = UpdateValue(xliffTarget, tuSourceShortcutKeys, locInfo.ShortcutKeys, locInfo, shortcutId);
				UpdateTransUnitComment(xliffTarget, tuSourceShortcutKeys, locInfo);
				UpdateTransUnitComment(xliffTarget, tuTargetShortcutKeys, locInfo);
				if (tuTargetShortcutKeys != null)
					lookupTarget[shortcutId] = tuTargetShortcutKeys;
				else if (lookupTarget.ContainsKey(shortcutId))
					lookupTarget.Remove(shortcutId);
			}

			// Save the tooltips
			TransUnit tuTargetToolTip = null;
			var tooltipId = locInfo.Id + kToolTipSuffix;
			if ((locInfo.UpdateFields & UpdateFields.ToolTip) == UpdateFields.ToolTip)
			{
				tuTargetToolTip = UpdateValue(xliffTarget, tuSourceToolTip, locInfo.ToolTipText, locInfo, tooltipId);
				UpdateTransUnitComment(xliffTarget, tuSourceToolTip, locInfo);
				UpdateTransUnitComment(xliffTarget, tuTargetToolTip, locInfo);
				if (tuTargetToolTip != null)
					lookupTarget[tooltipId] = tuTargetToolTip;
				else if (lookupTarget.ContainsKey(tooltipId))
					lookupTarget.Remove(tooltipId);
			}

			// Save the text
			TransUnit tuTargetText = null;
			if ((locInfo.UpdateFields & UpdateFields.Text) == UpdateFields.Text)
			{
				...
				text = text.Replace("@#$", _literalNewline);
				tuTargetText = UpdateValue(xliffTarget, tuSourceText, text, locInfo, locInfo.Id);
				UpdateTransUnitComment(xliffTarget, tuSourceText, locInfo);
				UpdateTransUnitComment(xliffTarget, tuTargetText, locInfo);
				if (tuTargetText != null)
					lookupTarget[locInfo.Id] = tuTargetText;
				else if (lookupTarget.ContainsKey(locInfo.Id))
					lookupTarget.Remove(locInfo.Id);
			}
		...
		void RemoveFromLookupCache(Dictionary<string, TransUnit> lookup, TransUnit tu)
		{
			if (tu == null)
				return;
			if (lookup.ContainsKey(tu.Id))
				lookup.Remove(tu.Id);
		}
		...
		{
			// One would think there would be a source TransUnit, but that isn't necessarily so
			...
			if (tuTarget != null)
			{
				...
				if (tuvTarg != null)
				{
					...
					if (String.IsNullOrEmpty(newValue))
					{
						...
						if ((tuTarget.Source == null || tuTarget.Source.Value == null) &&
							(tuTarget.Target == null || tuTarget.Target.Value == null))
						{
